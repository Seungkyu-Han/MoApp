...
import com.example.moapp.databinding.ActivityAddGroupScheduleBinding
...
class AddGroupScheduleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val binding = ActivityAddGroupScheduleBinding.inflate(layoutInflater)
        ...
        service.getGroupInfo(groupId, "Bearer $token").enqueue(object : Callback<Group> {
            ...
            override fun onResponse(call: Call<Group>, response: Response<Group>) {
                if (response.isSuccessful) {
                    ...
                    selectedDate = "${startCal.get(Calendar.YEAR)}-${startCal.get(Calendar.MONTH) + 1}-${startCal.get(Calendar.DAY_OF_MONTH)}"
                    ...
                    binding.dateBtn.setOnClickListener {
                        ...
                        val datePickerDialog = DatePickerDialog(this@AddGroupScheduleActivity, dateSetListener, cal.get(Calendar.YEAR),
                    ...
                    binding.addButton.setOnClickListener {
                        ...
                        val shareSchedulePostUserScheduleReq = shareSchedulePostUserScheduleReq(
                            selectedDate,
                            endTime,
                            groupId,
                            startTime
                        ...
                        Log.d("park", shareSchedulePostUserScheduleReq.toString())

                        service.addShareSchedule(shareSchedulePostUserScheduleReq, "Bearer $token").enqueue(
                            object : Callback<Unit> {
                                override fun onResponse(
                                    ...
                                    if (response.isSuccessful) {
                                        ...
                                        Toast.makeText(this@AddGroupScheduleActivity, "서버 오류",
                                ...
                                override fun onFailure(call: Call<Unit>, t: Throwable) {
                                    Toast.makeText(this@AddGroupScheduleActivity, "잠시후 다시 이용해주세요.",
                                        Toast.LENGTH_SHORT).show()
                                    Log.d("park", response.code().toString())
                                }
                            }
                    ...
                    Log.d("park", "${calStart.get(Calendar.YEAR)}-${calStart.get(Calendar.MONTH)}-${calStart.get(Calendar.DAY_OF_MONTH)}")
                } else {
                    Toast.makeText(this@AddGroupScheduleActivity, "그룹 정보를 가져올 수 없습니다.",
                        Toast.LENGTH_SHORT).show()
                    Log.d("park", response.code().toString())
                }
            }
